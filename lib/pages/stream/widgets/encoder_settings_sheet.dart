        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                children: EncoderFormat.values.map((f) {
                  return _choice(
                    ...
                    onTap: () => setState(() => _cfg = _cfg.copyWith(format: f)),
                  );
                }).toList(),
              ),
              const SizedBox(height: 20),
              ...
              _sliderRow(
                title: '编码质量（码率）',
                value: '${(_cfg.bitrate / 1000000).toStringAsFixed(1)} Mbps',
              ),
              _slider(
                value: _cfg.bitrate / 1000000,
                min: 0.5,
                max: 8.0,
                divisions: 30,
                onChanged: (v) => setState(() {
                  _cfg = _cfg.copyWith(
                    bitrate: ((v * 10).round() / 10 * 1000000).round(),
                  );
                }),
              ),
              const SizedBox(height: 8),

              // ── I-frame interval (H.264 only) ──
              if (_cfg.format.hasIFrameInterval) ...[
                _sliderRow(
                  title: 'I 帧间隔',
                  value: '${_cfg.iFrameIntervalSec} s',
                ),
                _slider(
